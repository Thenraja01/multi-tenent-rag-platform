...
logger = logging.getLogger("nexusrag.db")

# Primary Engine & Session Maker
engine_kwargs = {
...
async def get_db(organization_id: Optional[str] = None) -> AsyncGenerator[AsyncSession, None]:
    ...
    If organization_id is supplied, it sets PostgreSQL session variable `app.organization_id` for RLS.
    """
...
async def init_db() -> None:
    """Initialize database extensions and create tables with automatic local SQLite fallback."""
    ...
    try:
        ...
        logger.warning(f"Primary PostgreSQL connection failed ({e}), enabling auto-configured SQLite engine: sqlite+aiosqlite:///./nexusrag.db")
        engine = create_async_engine(
